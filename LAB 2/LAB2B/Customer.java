    ...
    public static boolean validate(String dob) {
        ...
        if(yyyy>=1000 && yyyy<=YearMonth.now().getYear()) {
            if (yyyy==YearMonth.now().getYear() && mm>=YearMonth.now().getMonthValue()) {
                ...
                //check days
                if((dd>=1 && dd<=daysinmonth[0]) && (mm==1 || mm==3 || mm==5 || mm==7 || mm==8 || mm==10 || mm==12)) {
                    return true;
                } else if((dd>=1 && dd<=daysinmonth[2]) && (mm==4 || mm==6 || mm==9 || mm==11)) {
                    return true;
                } else if((dd>=1 && dd<=daysinmonth[1]) && (mm==2)) {
                    return true;
                } else if(dd>=1 && dd==daysinmonth[3] && mm==2 && (yyyy%400==0) || ((yyyy%4==0) && (yyyy%100!=0))) {
                    return true;
                } else {
                    return false;
                }
